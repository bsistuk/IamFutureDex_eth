...
const Icone = (icon: string) => {
  const Icon = Icons[icon];
  return <Icon width="24px" mr="8px" />;
};
...
const PanelBody: React.FC<Props> = ({ isPushed, pushNav, isMobile, links, isDark }) => {
  ...
  return (
    <>
      <ContainerLogo>
        <Logo isDark={isDark} isPushed={isPushed} href={homeLink?.href ?? "/"} />
      </ContainerLogo>
      <Container>
        {links.map((entry) => {
                  ...
                  entry.items.map((item) => (
                    ...
                    >
                      <MenuLink href={item.href}>
                        {item.icon ? Icone(item.icon) : <></>}
                        <LinkLabel isActive={item.href === location.pathname} isPushed={isPushed}>
                          {item.label}
                        </LinkLabel>
                        {item.status && (
