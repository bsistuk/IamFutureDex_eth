...
contract DEXManagement is Ownable, Pausable, ReentrancyGuard {
    ...
    IIamFutureSwapRouter02 public defaultRouter;
    IIamFutureSwapRouter02 public externalRouter;
    ...
    /**
     * @param   _defaultRouter: default IamFuture router address
    ...
    ) 
    {
        ...
        defaultRouter = IIamFutureSwapRouter02(_defaultRouter);
        externalRouter = IIamFutureSwapRouter02(_externalRouter);
        TREASURY = _treasury;
        SWAP_FEE = _swapFee;
     ...
     * @return  bool: if pair is in IamFuture, return true, else, return false.
     */
    function isPairExists(address _tokenA, address _tokenB, uint _type) public view returns(bool){ 
        if (_type == 0) {
            return IIamFutureSwapFactory(defaultRouter.factory()).getPair(_tokenA, _tokenB) != address(0);
        }       
        return IIamFutureSwapFactory(externalRouter.factory()).getPair(_tokenA, _tokenB) != address(0);
    }
    ...
    function isPathExists(address _tokenA, address _tokenB, uint _type) public view returns(bool){   
        if(_type == 0) {
            return IIamFutureSwapFactory(defaultRouter.factory()).getPair(_tokenA, _tokenB) != address(0) || 
                (IIamFutureSwapFactory(defaultRouter.factory()).getPair(_tokenA, defaultRouter.WETH()) != address(0) && 
                IIamFutureSwapFactory(defaultRouter.factory()).getPair(defaultRouter.WETH(), _tokenB) != address(0));
        } else {
            return IIamFutureSwapFactory(externalRouter.factory()).getPair(_tokenA, _tokenB) != address(0) || 
                (IIamFutureSwapFactory(externalRouter.factory()).getPair(_tokenA, externalRouter.WETH()) != address(0) && 
                IIamFutureSwapFactory(externalRouter.factory()).getPair(externalRouter.WETH(), _tokenB) != address(0));
        }     
    }

    /**
     * @param   tokenA: InputToken Address to swap on IamFutureswap
     * @param   tokenB: OutputToken Address to swap on IamFutureswap
     * @param   _amountIn: Amount of InputToken to swap on IamFutureswap
     ...
     * @notice  Swap ERC20 token to ERC20 token on IamFutureswap
     */
    function swapExactTokensForTokens(
        ...
        IIamFutureSwapRouter02 selectedRouter = _type == 0 ? defaultRouter : externalRouter;
    ...
    /**
     * @param   token: OutputToken Address to swap on IamFutureswap
     * @param   _amountOutMin: The minimum amount of output tokens that must be received 
     *          for the transaction not to revert.
     * @param   to: Recipient of the output tokens.
     * @param   deadline: Deadline, Timestamp after which the transaction will revert.
     * @notice  Swap ETH to ERC20 token on IamFutureswap
     */
    ...
    /**
     * @param   token: InputToken Address to swap on IamFutureswap
     * @param   _amountIn: Amount of InputToken to swap on IamFutureswap
     * @param   _amountOutMin: The minimum amount of output tokens that must be received 
     *          for the transaction not to revert.
     * @param   to: Recipient of the output tokens.
     * @param   deadline: Deadline, Timestamp after which the transaction will revert.
     * @notice  Swap ERC20 token to ETH on IamFutureswap
     */
    ...
    function setDefaultRouter(address _newRouter) external onlyOwner whenNotPaused {
        ...
        defaultRouter = IIamFutureSwapRouter02(_newRouter);
    ...
    function setExternalRouter(address _newRouter) external onlyOwner whenNotPaused {
        ...
        externalRouter = IIamFutureSwapRouter02(_newRouter);
